  ...
  useEffect(() => {
    ...
    function onDiscoverUserDismissed(payload: DiscoverUserDismissedPayload) {
      ...
      >(
        ...
        (users) => users?.filter((user) => user.id !== payload.userId) ?? users,
      );
    }
    ...
    function onCallAccepted(payload: { callId: string; acceptedBy: string }) {
      useCallStore
        .getState()
        .markCallAccepted(payload.callId, payload.acceptedBy);
    }

    function onCallRejected(payload: { callId: string; rejectedBy: string }) {
      useCallStore
        .getState()
        .markCallRejected(payload.callId, payload.rejectedBy);
    }
    ...
    function onTypingStart(payload: TypingPayload) {
      useTypingStore
        .getState()
        .startTyping(payload.conversationId, payload.userId);
    }

    function onTypingStop(payload: TypingPayload) {
      useTypingStore
        .getState()
        .stopTyping(payload.conversationId, payload.userId);
    }

    function onMessageReceived(message: Message) {
      queryClient.setQueryData<Message[]>(
        queryKeys.messages.byConversation(message.conversationId),
        (messages) => {
    ...
    function onMessageDeleted(payload: {
      ...
      queryClient.setQueryData<Message[]>(
        queryKeys.messages.byConversation(payload.conversationId),
        (messages) =>
    ...
    socket.on("discover:user-dismissed", onDiscoverUserDismissed);
    ...
    return () => {
      ...
      socket.off("discover:user-dismissed", onDiscoverUserDismissed);
      ...
      socket.off("presence:update", onPresenceUpdated);
    };
  }, [queryClient]);
