...
export default function SocketProvider({ children }: Props) {
  ...
  useEffect(() => {
    ...
    function onPresenceUpdated(payload: PresencePayload) {
      if (!payload.userId) {
        return;
      }
    ...
    function onTypingUsers(payload: TypingPayload) {
    ...
    function onReceiveMessage(message: Message) {
      ...
      queryClient.invalidateQueries({
        queryKey: queryKeys.messages.list(message.conversationId),
      });
    ...
    function onMessageDelivered(payload: {
      ...
      updateMessageStatus(payload.messageId, "delivered", payload.serverId);
    }

    function onMessageSeen(payload: { messageId: string; serverId?: string }) {
      updateMessageStatus(payload.messageId, "read", payload.serverId);
    }
    ...
    function onConversationUpdated(conversation: Conversation) {
      queryClient.invalidateQueries({
      ...
      updateConversationMessage(
        conversation.id,
        conversation.latestMessage || "New activity",
      );
    }
    ...
    function onDiscoverUserDismissed(payload: DiscoverUserDismissedPayload) {
      ...
      >(
        ...
        (users) => users?.filter((user) => user.id !== payload.userId) ?? [],
      );
    }
    ...
    function onCallRejected(payload: { callId?: string }) {
      useCallStore.getState().handleCallRejected(payload);
    }

    function onCallEnded(payload: { callId?: string; reason?: string }) {
    ...
    socket.on(SOCKET_EVENTS.DISCOVER_USER_DISMISSED, onDiscoverUserDismissed);
