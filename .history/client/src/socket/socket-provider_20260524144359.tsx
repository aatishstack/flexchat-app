...
import { useCallStore, type CallSession } from "@/store/call-store";
...
type DiscoverUserDismissedPayload = { userId?: string };

type CallLifecyclePayload = { callId?: string; reason?: string };
...
type CallErrorPayload = { callId?: string; message?: string };

const CONVERSATION_UPDATE_DEDUPE_TTL_MS = 2 * 60 * 1000;
const PENDING_NOTIFICATION_CONVERSATION_KEY = "flexchat:pending-conversation";
const recentConversationUpdates = new Map<string, number>();

function getConversationNameFromCache(
  cache: ConversationQueryCache,
  conversationId: string,
) {
  if (!cache) return "New message";
  const conversations = Array.isArray(cache)
    ? cache
    : "pages" in cache
      ? cache.pages.flatMap((page) => page.conversations)
      : [];
  return (
    conversations.find((c) => c.id === conversationId)?.name ?? "New message"
  );
}
...
function hasSeenConversationUpdate(messageId?: string) {
  if (!messageId) return false;
  const now = Date.now();
  recentConversationUpdates.forEach((expiresAt, key) => {
    if (expiresAt <= now) recentConversationUpdates.delete(key);
  });
  if (recentConversationUpdates.has(messageId)) return true;
...
export default function SocketProvider({
  ...
  const setTypingUsers = useSocketStore((s) => s.setTypingUsers);
  const updateMessageStatus = useSocketStore((s) => s.updateMessageStatus);
  const setConnectionError = useSocketStore((s) => s.setConnectionError);
  const resetPendingMessageFlights = useSocketStore(
    (s) => s.resetPendingMessageFlights,
  );
  const recoverSocketConnection = useSocketStore(
    (s) => s.recoverSocketConnection,
  );
  const currentSocket = useSocketStore((s) => s.socket);
  ...
  useEffect(() => {
    ...
    function onConnect() {
      ...
      if (useCallStore.getState().currentCall) {
        useCallStore.setState({ networkState: "connecting", error: null });
      }
      ...
      if (socketState.activeConversationId) {
        void queryClient.refetchQueries({
          queryKey: queryKeys.messages.list(socketState.activeConversationId),
        });
      }
    }
    ...
    function onDisconnect(reason: string) {
      ...
      if (useCallStore.getState().currentCall) {
        useCallStore.setState({ networkState: "reconnecting" });
      }
    ...
    function onPresenceUpdated(payload: PresenceUpdatedPayload) {
      ...
      queryClient.setQueryData<ConversationQueryCache>(
        ...
        (cache) => {
          if (!cache || !payload.lastSeenAt) return cache;
          const updateMembers = (conversation: Conversation): Conversation => ({
            ...conversation,
            members:
              conversation.members?.map((member) =>
                member.id === payload.userId
                  ? { ...member, lastSeenAt: payload.lastSeenAt ?? member.lastSeenAt }
                  : member,
              ) ?? conversation.members,
          });
          if (Array.isArray(cache)) return cache.map(updateMembers);
          if ("pages" in cache) {
            return {
              ...cache,
    ...
    function onReceiveMessage(message: Message) {
      ...
      if (socket.connected && isRemoteMessage) {
    ...
    function onConversationUpdated(payload: ConversationUpdatedPayload) {
      ...
      if (hasSeenConversationUpdate(payload.messageId)) return;
      ...
      const shouldIncrementUnread = isRemoteMessage && !conversationIsActive;

      const cache =
        queryClient.getQueryData<ConversationQueryCache>(
          queryKeys.conversations.all,
        );
      const conversationName = getConversationNameFromCache(
        cache,
        payload.conversationId,
      );
      const conversationWasCached = hasConversationInCache(
        cache,
        payload.conversationId,
      );
      ...
      queryClient.setQueryData<ConversationQueryCache>(
        ...
        (c) =>
          updateConversationInQueryCache(
            ...
            payload.conversationId ?? "",
            (conversation) => {
              nextUnreadCount = conversationIsActive
                ? 0
              ...
              return {
                ...
                lastActivityAt:
                  payload.createdAt ?? conversation.lastActivityAt,
              };
            },
          ),
      );

      updateConversationMessage(payload.conversationId, latestMessage, {
    ...
    function onMessageDelivered(receipt: MessageReceipt) {
      queryClient.setQueriesData<MessageQueryCache>(
        { queryKey: ["messages"] },
        (cache) =>
          updateMessageStatusInQueryCache(cache, receipt, "delivered"),
      );
      updateMessageStatus(
        receipt.messageId,
    ...
    function onMessageSeen(receipt: MessageReceipt) {
      queryClient.setQueriesData<MessageQueryCache>(
        { queryKey: ["messages"] },
        (cache) => updateMessageStatusInQueryCache(cache, receipt, "read"),
      );
      updateMessageStatus(receipt.messageId, "read", receipt.serverId);
    }

    function onConversationError(payload: ConversationErrorPayload) {
      const activeConversationId =
        useSocketStore.getState().activeConversationId;
      if (
        payload.conversationId &&
      ...
      setConnectionError(payload.message ?? "Conversation unavailable");
    }
    ...
    function onConversationArchiveUpdated(
      ...
      useConversationStore.setState((state) => ({
        conversationPatches: {
          ...
          [payload.conversationId ?? ""]: {
            ...state.conversationPatches[payload.conversationId ?? ""],
            archivedAt: payload.archivedAt ?? null,
          },
        },
      }));
    ...
    function onConversationDeleted(payload: ConversationDeletedPayload) {
      ...
      queryClient.setQueryData<ConversationQueryCache>(
        queryKeys.conversations.all,
        (cache) => removeConversationFromQueryCache(cache, conversationId),
      );
      useConversationStore.setState((state) => {
        const conversationPatches = { ...state.conversationPatches };
      ...
      useConversationStore.setState((state) => ({
        conversationPatches: {
          ...
          [payload.conversationId ?? ""]: {
            ...state.conversationPatches[payload.conversationId ?? ""],
            ...(payload.scope === "me"
              ? { localThemeId: payload.themeId ?? null }
              : { localThemeId: null, sharedThemeId: payload.themeId ?? null }),
            themeUpdatedAt:
              payload.updatedAt ??
    ...
    function onStoryCreated(story: Story) {
      if (!story?.id) return;
      queryClient.setQueryData<Story[]>(queryKeys.stories.all, (stories) => [
        story,
        ...(stories ?? []).filter((item) => item.id !== story.id),
      ]);
    }

    function onStoryViewed(payload: StoryViewedPayload) {
      ...
      void queryClient.invalidateQueries({
        queryKey: [...queryKeys.stories.all, payload.storyId, "viewers"],
      });
    }

    function onStoryDeleted(payload: StoryDeletedPayload) {
      if (!payload.storyId) return;
      queryClient.setQueryData<Story[]>(
        queryKeys.stories.all,
        (stories) =>
          (stories ?? []).filter((story) => story.id !== payload.storyId),
      );
    }

    function onAccountDeleted(payload: AccountDeletedPayload) {
      if (!payload.userId) return;
      const currentUserId = useAuthStore.getState().user?.id;
    ...
    function onUserUpdated(payload: UserUpdatedPayload) {
      ...
      if (!updatedUser?.id || !updatedUser.username) return;
      ...
      if (updatedUserId === useAuthStore.getState().user?.id) {
        useAuthStore
          .getState()
          .updateUser({ ...publicUpdate, email: updatedUser.email });
      ...
      >({ queryKey: ["users"] }, (users) =>
        users?.map((user) =>
          user.id === updatedUserId
            ? { ...user, username: updatedUsername, avatar: updatedAvatar }
            : user,
        ) ?? users,
      );
    ...
    function onDiscoverUserDismissed(payload: DiscoverUserDismissedPayload) {
      if (!payload.userId) return;
      queryClient.setQueriesData
        { id: string; username: string; avatar?: string | null }[]
      >({ queryKey: ["users", "discover"] }, (users) =>
        users?.filter((user) => user.id !== payload.userId) ?? users,
      );
    }
    ...
    function onCallAccepted(call: CallSession) {
      addCallNotification({
      ...
      useCallStore.getState().handleCallAccepted(call);
    }

    function onCallRejected(payload: CallLifecyclePayload) {
      if (payload.callId) {
        addCallNotification({
    ...
    function onCallCanceled(payload: CallLifecyclePayload) {
      useCallStore.getState().handleCallCanceled(payload);
    }

    function onCallEnded(payload: CallLifecyclePayload) {
      if (
        payload.callId &&
        ...
        addCallNotification({
          ...
          title:
            payload.reason === "missed" ? "Missed call" : "Call ended",
          message:
            payload.reason === "unreachable"
    ...
    function onCallSignalRelay(
      ...
      useCallStore.getState().handleCallSignal({
    ...
    function onCallError(payload: CallErrorPayload) {
      useCallStore.getState().handleCallError(payload);
    }

    // ─── Network event handlers ───────────────────────────────────────────
    // IMPORTANT: These are the ONLY network listeners in the entire app.
    // socket.ts has NO window listeners. Having listeners in both places
    // causes double-connect which corrupts Engine.IO state → forever "connecting".

    function handleOffline() {
      console.warn("[FlexChat Socket] browser offline", {
        network: getSocketNetworkDiagnostics(),
      });
      resetPendingMessageFlights();
      useSocketStore.setState({
    ...
    function handleOnline() {
      console.info("[FlexChat Socket] browser online", {
        network: getSocketNetworkDiagnostics(),
      });
      // 600ms delay: let network stack stabilize after WiFi/LTE switch.
      // Also guard with socket.active — if Engine.IO is already mid-handshake,
      // do not call recoverSocketConnection (it would cancel the in-progress attempt).
      setTimeout(() => {
        const s = useSocketStore.getState().socket;
        if (s && !s.connected && !s.active) {
          recoverSocketConnection("browser-online");
        } else if (s && !s.connected && s.active) {
          // Engine.IO is already attempting — let it finish, don't interrupt
          console.info("[FlexChat Socket] online recovery skipped — already attempting");
        }
      }, 600);
    }

    // NOTE: navigator.connection "change" listener is intentionally NOT here.
    // It fires on every signal fluctuation on mobile (20+ times during a
    // WiFi/LTE switch) and each fire would call recoverSocketConnection,
    // cancelling the previous in-progress handshake. The "online" event
    // alone is sufficient and fires exactly once after a network switch.

    const safeOnConnect = guardSocketHandler(SOCKET_EVENTS.CONNECT, onConnect);
    const safeOnDisconnect = guardSocketHandler(SOCKET_EVENTS.DISCONNECT, onDisconnect);
    const safeOnConnectError = guardSocketHandler(SOCKET_EVENTS.CONNECT_ERROR, onConnectError);
    const safeOnOnlineUsers = guardSocketHandler(SOCKET_EVENTS.ONLINE_USERS, onOnlineUsers);
    const safeOnPresenceUpdated = guardSocketHandler(SOCKET_EVENTS.PRESENCE_UPDATED, onPresenceUpdated);
    const safeOnReceiveMessage = guardSocketHandler(SOCKET_EVENTS.RECEIVE_MESSAGE, onReceiveMessage);
    const safeMergeRealtimeMessage = guardSocketHandler("message mutation", mergeRealtimeMessage);
    const safeOnConversationUpdated = guardSocketHandler(SOCKET_EVENTS.CONVERSATION_UPDATED, onConversationUpdated);
    const safeOnTypingUsers = guardSocketHandler(SOCKET_EVENTS.TYPING_USERS, onTypingUsers);
    const safeOnMessageDelivered = guardSocketHandler(SOCKET_EVENTS.MESSAGE_DELIVERED, onMessageDelivered);
    const safeOnMessageSeen = guardSocketHandler(SOCKET_EVENTS.MESSAGE_SEEN, onMessageSeen);
    const safeOnConversationError = guardSocketHandler(SOCKET_EVENTS.CONVERSATION_ERROR, onConversationError);
    const safeOnConversationArchiveUpdated = guardSocketHandler(SOCKET_EVENTS.CONVERSATION_ARCHIVE_UPDATED, onConversationArchiveUpdated);
    const safeOnConversationDeleted = guardSocketHandler(SOCKET_EVENTS.CONVERSATION_DELETED, onConversationDeleted);
    const safeOnConversationThemeUpdated = guardSocketHandler(SOCKET_EVENTS.CONVERSATION_THEME_UPDATED, onConversationThemeUpdated);
    const safeOnStoryCreated = guardSocketHandler(SOCKET_EVENTS.STORY_CREATED, onStoryCreated);
    const safeOnStoryViewed = guardSocketHandler(SOCKET_EVENTS.STORY_VIEWED, onStoryViewed);
    const safeOnStoryDeleted = guardSocketHandler(SOCKET_EVENTS.STORY_DELETED, onStoryDeleted);
    const safeOnAccountDeleted = guardSocketHandler(SOCKET_EVENTS.ACCOUNT_DELETED, onAccountDeleted);
    const safeOnUserUpdated = guardSocketHandler(SOCKET_EVENTS.USER_UPDATED, onUserUpdated);
    const safeOnDiscoverUserDismissed = guardSocketHandler(SOCKET_EVENTS.DISCOVER_USER_DISMISSED, onDiscoverUserDismissed);
    const safeOnCallIncoming = guardSocketHandler(SOCKET_EVENTS.CALL_INCOMING, onCallIncoming);
    const safeOnCallAccepted = guardSocketHandler(SOCKET_EVENTS.CALL_ACCEPTED, onCallAccepted);
    const safeOnCallRejected = guardSocketHandler(SOCKET_EVENTS.CALL_REJECTED, onCallRejected);
    const safeOnCallCanceled = guardSocketHandler(SOCKET_EVENTS.CALL_CANCELED, onCallCanceled);
    const safeOnCallEnded = guardSocketHandler(SOCKET_EVENTS.CALL_ENDED, onCallEnded);
    const safeOnCallOffer = guardSocketHandler(SOCKET_EVENTS.CALL_OFFER, onCallOffer);
    const safeOnCallAnswer = guardSocketHandler(SOCKET_EVENTS.CALL_ANSWER, onCallAnswer);
    const safeOnCallIceCandidate = guardSocketHandler(SOCKET_EVENTS.CALL_ICE_CANDIDATE, onCallIceCandidate);
    const safeOnCallError = guardSocketHandler(SOCKET_EVENTS.CALL_ERROR, onCallError);
    ...
    socket.on(SOCKET_EVENTS.CONVERSATION_ARCHIVE_UPDATED, safeOnConversationArchiveUpdated);
    socket.on(SOCKET_EVENTS.CONVERSATION_DELETED, safeOnConversationDeleted);
    socket.on(SOCKET_EVENTS.CONVERSATION_THEME_UPDATED, safeOnConversationThemeUpdated);
    ...
    socket.on(SOCKET_EVENTS.DISCOVER_USER_DISMISSED, safeOnDiscoverUserDismissed);
    ...
    return () => {
      ...
      socket.off(SOCKET_EVENTS.CONVERSATION_ARCHIVE_UPDATED, safeOnConversationArchiveUpdated);
      socket.off(SOCKET_EVENTS.CONVERSATION_DELETED, safeOnConversationDeleted);
      socket.off(SOCKET_EVENTS.CONVERSATION_THEME_UPDATED, safeOnConversationThemeUpdated);
      ...
      socket.off(SOCKET_EVENTS.DISCOVER_USER_DISMISSED, safeOnDiscoverUserDismissed);
