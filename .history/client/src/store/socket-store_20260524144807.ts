...
function schedulePendingRetry(
  ...
  const delayMs = Math.min(30_000, 1_500 * Math.max(1, attempts));
...
function trimEphemeralMessages(messages: Message[]) {
  ...
  return Array.from(byConversation.values()).flatMap((msgs) =>
    sortMessages(msgs).slice(-MAX_EPHEMERAL_MESSAGES_PER_CONVERSATION),
  );
}
...
export const useSocketStore = create<SocketState>((set, get) => ({
  ...
  connectSocket: (token) => {
    ...
    if (currentSocket?.connected && currentToken === nextToken) {
  ...
  recoverSocketConnection: (reason = "recovery", options) => {
    ...
    // Engine.IO is already mid-handshake — don't cancel it unless forced
    if (currentSocket?.active && !options?.force) {
      console.info("[FlexChat Socket] recovery skipped — already attempting", {
        reason,
      });
      return;
    }
  ...
  setOnlineUsers: (users) => {
    ...
    if (arraysEqual(get().onlineUsers, nextUsers)) return;
    set({ onlineUsers: nextUsers });
  },

  setTypingUsers: (users) => {
    const nextUsers = Array.from(new Set(users)).sort();
    if (arraysEqual(get().typingUsers, nextUsers)) return;
    set({ typingUsers: nextUsers });
  },
        ...
        (error: Error | null, ack?: ServerMessageAck) => {
          ...
          if (error || !ack || !ack.ok) {
            console.warn("[FlexChat Socket] message ACK failed", {
              ...
              transport: get().socket?.io.engine?.transport?.name,
              error:
                ack?.error ?? error?.message ?? "Message was not acknowledged",
            });

            if (error && activeSocket === get().socket) {
              get().recoverSocketConnection(`message_ack:${error.message}`, {
                force: true,
              });
            }
  ...
  joinConversation: (conversationId) => {
    ...
    if (
      ...
      activeSocket.emit(SOCKET_EVENTS.LEAVE_CONVERSATION, {
        conversationId: previousConversationId,
      });
    }
    ...
    activeSocket.emit(SOCKET_EVENTS.JOIN_CONVERSATION, { conversationId });
  },
  ...
  leaveConversation: (conversationId) => {
    ...
    if (get().activeConversationId === conversationId) {
      set({ activeConversationId: null, typingUsers: [] });
    }

    if (!activeSocket?.connected) return;

    activeSocket.emit(SOCKET_EVENTS.STOP_TYPING, { conversationId });
    activeSocket.emit(SOCKET_EVENTS.LEAVE_CONVERSATION, { conversationId });
  },

  rejoinActiveConversation: () => {
    const conversationId = get().activeConversationId;
    const activeSocket = get().socket;
    if (!conversationId || !activeSocket?.connected) return;
    activeSocket.emit(SOCKET_EVENTS.JOIN_CONVERSATION, { conversationId });
  },

  sendMessage: (data) => {
    const text = data.text.trim();
    if (!text && !data.attachment && !data.audio && !data.mediaId) return;
  ...
  startTyping: (conversationId) => {
    const activeSocket = get().socket;
    if (!activeSocket?.connected) return;
    activeSocket.emit(SOCKET_EVENTS.START_TYPING, { conversationId });
  },
