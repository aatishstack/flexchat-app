...
function closeCall(
  ...
  participantIds(call).forEach((userId) => {
    const timerKey = userCallTimerKey(
      callId,
      userId
    );
    const timer =
...
async function canCallTarget(
  ...
  const [callerAllowed, targetAllowed] =
    await Promise.all([
      isConversationMember(
        callerId,
        conversationId
...
export function registerCallHandlers(
  ...
  const activeCallId =
    activeCallByUser.get(userId);

  if (activeCallId) {
    const activeCall =
      calls.get(activeCallId);
  ...
  socket.on(
    ...
    async (
      ...
      const parsed =
        callInviteSchema.safeParse(payload);
      ...
      if (
        activeCallByUser.has(userId) ||
        activeCallByUser.has(targetUserId)
      ) {
        acknowledge(ack, {
          ok: false,
          error:
  ...
  socket.on(
    ...
    (
      ...
      const parsed =
        callIdSchema.safeParse(payload);
  ...
  socket.on(
    ...
    (payload: unknown) => {
      ...
      const otherParticipant =
        call &&
        getOtherParticipant(call, userId);
  ...
  socket.on(
    ...
    (payload: unknown) => {
      const parsed =
        callSignalSchema.safeParse(payload);
      ...
      io.to(`user:${targetUserId}`).emit(
        SOCKET_EVENTS.CALL_SIGNAL_RELAY,
        {
          callId: call.id,
          fromUserId: userId,
