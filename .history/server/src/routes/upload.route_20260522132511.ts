...
const VIDEO_UPLOAD_LIMIT_BYTES = 25 * 1024 * 1024;
...
function getAllowedMediaType(mimeType: string, extension: string) {
  const mediaType = allowedMediaTypes.get(mimeType.toLowerCase());

  if (!mediaType || !mediaType.extensions.includes(extension)) {
    return null;
  }
...
export async function uploadRoutes(app: FastifyInstance) {
  app.post(
    ...
    async (request, reply) => {
      ...
      if (!isFileSignatureAllowed(data.mimetype, header)) {
        await removeFileIfExists(filepath);
