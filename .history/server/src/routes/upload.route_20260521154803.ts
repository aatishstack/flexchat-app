...
function hasAsciiAt(
  ...
  return (
    ...
    buffer
      ...
      .toString("ascii") === value
  );
}

function hasOneOfAsciiAt(
  buffer: Buffer,
  offset: number,
  values: string[]
) {
  return values.some((value) =>
    hasAsciiAt(buffer, offset, value)
  );
}
...
function hasMp4FamilyBrand(buffer: Buffer) {
  ...
  return [
    ...
    brandWindow.includes(brand)
  );
}

function isFileSignatureAllowed(
  mimeType: string,
  header: Buffer
) {
  switch (mimeType.toLowerCase()) {
    ...
    case "image/gif":
      return (
        hasAsciiAt(header, 0, "GIF87a") ||
        hasAsciiAt(header, 0, "GIF89a")
      );
    case "image/jpeg":
      return (
        header[0] === 0xff &&
    ...
    case "audio/mpeg":
      return (
        hasAsciiAt(header, 0, "ID3") ||
        hasMp3FrameSync(header)
      );
    case "audio/mp4":
      return hasMp4FamilyBrand(header);
    case "audio/ogg":
